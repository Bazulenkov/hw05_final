...
class TestHW04(TestCase):
    ...
    def test_new_post_display(self):
        ...
        сайта (index), на персональной странице пользователя (profile), и на
        отдельной странице поста (post)"""
        post = Post.objects.create(
            text=(
                "You are talking about things I have not done yet "
                "in the past tense."
            ),
            author=self.user,
        )
    ...
    def test_post_edit(self):
        ...
        self.client.post(
            f"/{self.user.username}/{post.id}/edit/",
            {"text": "text3"},
            follow=True,
        )
        url_list = [
...
class TestSprint6(TestCase):
    ...
    def test_post_with_image(self):
        """Проверяем страницу конкретной записи с картинкой: на странице есть
        тег <img>"""
    ...
    def test_post_with_image_display(self):
        """Проверяем, что на главной странице, на странице профайла и на
    ...
    def test_wrong_image(self):
        """Проверяем, что срабатывает защита от загрузки файлов не-графических
        форматов"""
        self.client.force_login(self.user)
    ...
    def test_follow(self):
        ...
        self.client.get(
            f"/{user.username}/unfollow/", {"username": user.username}
        )
    ...
    def test_comment(self):
        ...
        # Проверяем, что незарегистрированный пользоыватель не может
        # комментировать посты
        self.client.post(
        ...
        self.assertFalse(
            self.post.comment_post.filter(text="test_comment").exists()
        )
        user = User.objects.create_user(
            username="terminator",
            email="terminator@skynet.com",
